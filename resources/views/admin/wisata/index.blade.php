    ...
    <div class="group-data-[sidebar-size=lg]:ltr:md:ml-vertical-menu group-data-[sidebar-size=lg]:rtl:md:mr-vertical-menu group-data-[sidebar-size=md]:ltr:ml-vertical-menu-md group-data-[sidebar-size=md]:rtl:mr-vertical-menu-md group-data-[sidebar-size=sm]:ltr:ml-vertical-menu-sm group-data-[sidebar-size=sm]:rtl:mr-vertical-menu-sm pt-[calc(theme('spacing.header')_*_1)] pb-[calc(theme('spacing.header')_*_0.8)] px-4 group-data-[navbar=bordered]:pt-[calc(theme('spacing.header')_*_1.3)] group-data-[navbar=hidden]:pt-0 group-data-[layout=horizontal]:mx-auto group-data-[layout=horizontal]:max-w-screen-2xl group-data-[layout=horizontal]:px-0 group-data-[layout=horizontal]:group-data-[sidebar-size=lg]:ltr:md:ml-auto group-data-[layout=horizontal]:group-data-[sidebar-size=lg]:rtl:md:mr-auto group-data-[layout=horizontal]:md:pt-[calc(theme('spacing.header')_*_1.6)] group-data-[layout=horizontal]:px-3 group-data-[layout=horizontal]:group-data-[navbar=hidden]:pt-[calc(theme('spacing.header')_*_0.9)]">
        <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
            <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                <div class="grow">
                    <h5 class="text-16">Tempat Wisata</h5>
                </div>
            ...
            <div class="grid grid-cols-12 2xl:grid-cols-12 gap-x-5">
                <div class="col-span-12 card">
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full whitespace-nowrap">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y text-base border-slate-200 dark:border-zink-500">{{ $wisata->nama_tempat }}</td>
                                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y text-base border-slate-200 dark:border-zink-500">{{ \Illuminate\Support\Str::limit($wisata->deskripsi, 45) }}</td>
                                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y text-base border-slate-200 dark:border-zink-500">{{ \Illuminate\Support\Str::limit($wisata->alamat, 20) }}</td>
                                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y text-base border-slate-200 dark:border-zink-500">{{ $wisata->latitude }}</td>
                                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y text-base border-slate-200 dark:border-zink-500">{{ $wisata->longitude }}</td>
                                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y text-base border-slate-200 dark:border-zink-500">{{ $wisata->status }}</td>
                                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500">
                                        <div class="flex gap-3">
                                            <a href="{{ route('wisata.edit', $wisata->id_tempat) }}" class="flex items-center justify-center transition-all duration-200 ease-linear rounded-md size-8 edit-item-btn bg-slate-100 text-slate-500 hover:text-custom-500 hover:bg-custom-100 dark:bg-zink-600 dark:text-zink-200 dark:hover:bg-custom-500/20 dark:hover:text-custom-500"><i data-lucide="pencil" class="size-4"></i></a>
                                            <button type="button" onclick="openDeleteModal('{{ $wisata->id_tempat }}')" class="flex items-center justify-center transition-all duration-200 ease-linear rounded-md size-8 remove-item-btn bg-slate-100 text-slate-500 hover:text-red-500 hover:bg-red-100 dark:bg-zink-600 dark:text-zink-200 dark:hover:bg-red-500/20 dark:hover:text-red-500"><i data-lucide="trash-2" class="size-4"></i></button>
                                             <!-- Hidden Delete Form -->
                                             
                                            <form id="delete-form-{{ $wisata->id_tempat }}" action="{{ route('wisata.destroy', ['id' => $wisata->id_tempat ]) }}" method="POST"  class="hidden">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
                                ...
                                <div class="alert alert-danger">
                                    Data Tempat Wisata Masih Kosong.
                                </div>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="flex flex-col items-center gap-4 px-4 mt-4 mb-5 md:flex-row" id="pagination-element">
                        <div class="grow">
                            <p class="text-slate-500 dark:text-zink-200">Showing <b class="showing">{{ $wisatas->count() }}</b> of <b class="total-records">{{ $wisatas->total() }}</b> Results</p>
                        </div>

                        <div class="col-sm-auto mt-sm-0">
                            {{ $wisatas->appends(request()->query())->links('vendor.pagination.tailwind-custom') }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div id="deleteModal" class="fixed inset-0 flex items-center justify-center z-50 hidden mx-6">
        ...
        <div class="modal-container bg-white dark:bg-zink-800 md:w-1/3 mx-4 md:mx-auto rounded shadow-lg z-50 overflow-y-auto px-6">
            <div class="modal-body py-4 text-center">
                ...
                <p class="text-gray-600 dark:text-zink-400">Apakah anda yakin menghapus lokasi wisata ?</p>
